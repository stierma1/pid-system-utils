...
module.exports = async function(config){
  var pid = await System.promote(RoutingMaster, "emitter", {config: config.router.routes, name:config.router.name || "routing-master"});
  await System.register(config.router.name || "routing-master", pid);
  ...
  var pid = await System.resolve(config.router.name || "routing-master");
  System.send(pid, [{}])
}
